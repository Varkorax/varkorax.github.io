...
function buildLeftNavSequence() {
  ...
  $$('.chap-item[data-href]', chapList).forEach(li => {
    ...
    if (!seq.some(s => s.base === dhBase)) seq.push({ href: dh, base: dhBase, label: dh });
  });
...
prevPageBtn && prevPageBtn.addEventListener('click', (e) => {
  ...
  if (cur > 0 && seq[cur - 1] && seq[cur - 1].href) window.location.href = seq[cur - 1].href;
});
...
nextPageBtn && nextPageBtn.addEventListener('click', (e) => {
  ...
  let cur = findCurrentNavIndex(seq);
  if (cur === -1) cur = seq.length ? 0 : -1;
  if (cur >= 0 && cur < seq.length - 1 && seq[cur + 1] && seq[cur + 1].href) window.location.href = seq[cur + 1].href;
});

/* ========================================================================
   PANELS & RIGHT CONTENTS
   ======================================================================== */
function getPanels() { return contentBox ? $$('.panel', contentBox) : []; }
...
function setPanelCaret(panel) {
  ...
  header && header.setAttribute('aria-expanded', open ? 'true' : 'false');
...
function openPanel(index, opts = { closeOthers: false, scrollOnOpen: true, behavior: 'smooth' }, cb = null) {
  ...
  if (opts.scrollOnOpen) navigateToElement(target, 0, opts.behavior || 'smooth');
...
function buildContents() {
  ...
  panels.forEach((p, i) => {
    ...
    a.addEventListener('click', (e) => {
      ...
      openPanel(i, { closeOthers: false, scrollOnOpen: true, behavior: 'smooth' }, () => navigateToElement(p, 0, 'smooth'));
    });
    li.appendChild(a);
  ...
  try {
    ...
    try {
      ...
      if (!isPageRoot) {
        ...
        catch { scrollRoot.scrollTop = desiredTop; }
        return;
      }
    } catch { /* fallthrough */ }

    try { link.scrollIntoView({ block: 'nearest', inline: 'nearest', behavior: 'smooth' }); }
    catch { try { link.scrollIntoView(true); } catch { /* ignore */ } }
  } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $$('.chap-link, .sub-list a, .chap-list a').forEach(a => {
    const hrefBase = basename(a.getAttribute('href') || '');
    if (hrefBase === currentFile) {
      ...
      parentLi && parentLi.classList.add('active');
    }
  });
  ...
  if (parent) {
    ...
    header && header.addEventListener('keydown', (ev) => { if (ev.key === 'Enter' || ev.key === ' ') { toggle(ev); } });
  }
  ...
  */
  /* --- Cascading highlight for LE pages (full-width trail; no color change) --- */
	{
	  // If your filenames are different, edit this array (order matters).
	  const LE_PAGE_ORDER = ['me1.html', 'me2.html', 'me3.html'];

	  // get last path segment; handles nested paths, querystrings
...
(function initShowHideToggles() {
  ...
  buttons.forEach(btn => {
    ...
    // initial state from DOM (aria-hidden or class)
    ...
    btn.textContent = initiallyOpen ? 'Hide Details' : 'Show Details';
...
window.__STEP = {
  ...
  findCurrentNavIndex: () => findCurrentNavIndex(buildLeftNavSequence())
};
